...
{
	int i;
	size = (size + 1) & 0xfe;	// always even value
...
int mc_pmu_set_ema(int ema)
{
	...
	dst[idx++] = MC_PMU_SET_INSTRUCTION(PMU_CMD_REG_PWRDNREQN, PMU_OP_0, 8);
	dst[idx++] = MC_PMU_SET_INSTRUCTION(PMU_CMD_WAIT_PWRDNACKN, PMU_OP_0, 8);
	dst[idx++] = MC_PMU_SET_INSTRUCTION(PMU_CMD_REG_CLKOFF, PMU_OP_ALL_1, 1);
	dst[idx++] = MC_PMU_SET_INSTRUCTION(PMU_CMD_REG_EMA, PMU_OP_2, 1);
	dst[idx++] = MC_PMU_SET_INSTRUCTION(PMU_CMD_REG_CLKOFF, PMU_OP_ALL_0, 1);
	dst[idx++] = MC_PMU_SET_INSTRUCTION(PMU_CMD_REG_PWRDNREQN, PMU_OP_1, 8);
	...
	do {
		register volatile unsigned int delay = 100000;
		while (delay--);
	} while (mc_pmu_is_busy());
...
void cpupmu_initialize(void)
{
	...
	cpu_pwr_up     = cpu_power_up_sequence(&PMU_PROGRAM_BUFFER[0], 0, 1);
	...
	mc_pmu_download_program(&PMU_PROGRAM_BUFFER[0], cluster_pwr_dn);
}
